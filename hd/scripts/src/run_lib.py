...
def run(directory, test, test_name, args):
    ...
    cmd = "iverilog -g2005-sv" + flags + "../tests/" + test + " ../sim/hd_simenv.v\
     ../rtl/hd_top.v ../sim/hd_clk_rst_ctrl.v\
     ../sim/hd_latency_match.v ../../he/rtl/he_top.v\
     ../../he/rtl/he_parity_gen.v ../sim/hd_data_chk.v\
     ../sim/hd_noisy_channel.v ../../he/sim/he_data_gen.v && ./a.out"
...
def make_noisy_heredoc():
    heredoc_seed = random.randrange(2**32)
    contents = "localparam heredoc_seed = " + str(heredoc_seed) + ";\n"
    fptr = open("hd_noisy_heredoc.v", "w")
    fptr.write(contents)
    fptr.close()
